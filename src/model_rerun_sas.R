     choice <- function(){
        ...
        if(modif=="MF"){
            choice <- function(){
                  x <- readline("Which measure of fit should be used for calibration (SSE<- squared sum of error, NSSE<- Nash Sutcliffe, SSPE<- squared sum of prediction error, MPE<- mean prediction error )? ")  
                  if(x!="SSE" &  x!="NSSE" &  x!="SSPE" &  x!="MPE") readline("The answer can only be one of the following: SSE<- squared sum of error, NSSE<- Nash Sutcliffe, SSPE<- squared sum of prediction error, MPE<- mean prediction error ") else x <- x
                 }
                obj_fun<-choice()
            }
