...
while (obj_fun!="SSE" &  obj_fun!="NSSE" &  obj_fun!="SSPE" &  obj_fun!="MPE") {obj_fun<-dlgInput("The answer can only be one of the following: NSE<- Nash Sutcliffe, MPE<- mean prediction error", Sys.info()["user"])$res}
...
time_unit<-dlgInput("What is the duration of each input time step (1 <- annual time steps, 12 <- monthly time steps, 55 <- weekly time steps, 365 <- daily time steps)", Sys.info()["user"])$res
while (!(time_unit=="1" |  time_unit=="12" | time_unit=="55" | time_unit=="365")) {time_unit<-dlgInput("The answer can only be 1, 12, 55 or 365", Sys.info()["user"])$res}
time_unit <- as.numeric(time_unit) # Define the duration of each time step (used mainly for tritium decay). 
...
time_unit<-12 # Define the duration of each input time step (used mainly for tritium decay). 1 <- annual time steps, 12 <- monthly time steps, 55 <- weekly time steps, 365 <- daily time steps
