...
##################################
# work space definition and setup #
##################################
...
data_input_dir<-paste(folder, "data/raw", sep = "")
...
for(k in 1:20) { # up to 20 runs can be done, changing one element each time (for instance the LPM model used, the objective function, etc..)
    ...
    if(time_step!="R") source("input_interpolation_version0_4.R") # gaps in the input are interpolated, unless the user has chosen to work directly with the raw data
    ...
    if(time_step=="R") alpha=1 # sets the infiltration coefficient to "1" (i.e. no weigthing of the input summer months) if the user has decided to use raw data. !!! By using the raw data option, the input will not be weighted in WITS.
