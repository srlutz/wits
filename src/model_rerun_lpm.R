        ...
        while (obj_fun!="SSE" &  obj_fun!="NSE" &  obj_fun!="SSPE" &  obj_fun!="MPE") {obj_fun<-dlgInput("The answer can only be one of the following: SSE<- squared sum of error, NSE<- Nash Sutcliffe, SSPE<- squared sum of prediction error, MPE<- mean prediction error", Sys.info()["user"])$res}
        }
