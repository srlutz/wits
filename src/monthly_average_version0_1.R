...
if(tracer=="ST" | tracer=="H3+ST" | weight_input==2) {
    ...
    dummy1=format(as.Date(date_decimal(stable_output_date)),"%Y-%m") # grabs the dates for which output observations are available and removes the day so that monthly data can be aggregated.
...
if(tracer=="H3" | tracer=="H3+ST") {
    ...
    dummy1=format(as.Date(date_decimal(h3_output_date)),"%Y-%m") # grabs the dates for which output observations are available and removes the day so that monthly data can be aggregated.
...
if(flow_dyn=="TRAN") {
   ...
   # median=aggregate(ts_val,list(dummy1),median) #calculates the median output concentration for each month with available data.
    median=aggregate(ts_val,list(dummy1),sum) #calculates the sum of all fluxes for each month with available data.
    dummy2=as.Date(paste(as.character(median$Group.1),"-15",sep=""),format="%Y-%m-%d") # adds the 15th of each month to the date vector to match the date of the input entries.
